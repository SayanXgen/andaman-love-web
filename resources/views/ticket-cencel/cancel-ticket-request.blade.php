...
@section('content')
    ...
    <form action="{{ url('/ticket-cancellation-preview') }}" method="post" onsubmit="return validateForm()">
        @csrf
        <div class="container ">
            <div class="row justify-content-center mt-5">
                <div class="col-12 col-md-8 col-lg-3 text-center">
                    <label for="cancel_for_ferry" class="form-label fs-5 text-secondary">
                        <b>For Ferry</b>
                    </label>
                    <div id="pnr_id">
                        <input type="tel" name="pnr_id" class="form-control" placeholder="Enter PNR Id/Booking Id" value="{{ old('pnr_id') }}" id="pnrIdInput">
                        <span id="bookingError" class="error-message text-danger"></span>
                    </div>
                </div>
            </div>
...
<script>
    function validateForm() {
        const pnrIdInput = document.getElementById('pnrIdInput');
        const bookingError = document.getElementById('bookingError');
        const pnrIdValue = pnrIdInput.value.trim();

        // Clear previous error messages
        bookingError.textContent = '';

        // Check if PNR Id is empty
        if (pnrIdValue === '') {
            bookingError.textContent = 'PNR Id/Booking Id is required.';
            return false;
        }
